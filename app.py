...
def gerar_target(df, horas_futuro):
    df['Future_Close'] = df['Close'].shift(-horas_futuro)
...
def treinar_modelo(df, horas_futuro):
    df = criar_variaveis(df)
    df = gerar_target(df, horas_futuro)
...
# Interface Streamlit
horas_opcao = st.selectbox("Selecione o horizonte da previsão:",
                           [3, 6, 12, 24],
                           index=0,
...
with st.spinner("Carregando e treinando modelo melhorado..."):
    df = carregar_dados()
    model, scaler, df_proc, features, acc, relatorio = treinar_modelo(df, horas_opcao)

# Resultados
st.write("Acurácia do modelo:", f"{acc * 100:.2f}%")
st.write("Precisão por classe (quanto maior, melhor):")
st.json({
...
if st.button("Prever próximo movimento"):
    ...
    if pred == 2:
        st.success(f"Previsão para {horas_opcao}h: **ALTA > 1%**")
    elif pred == 0:
        st.error(f"Previsão para {horas_opcao}h: **BAIXA > 1%**")
    else:
        st.info(f"Previsão para {horas_opcao}h: **MERCADO ESTÁVEL**")
